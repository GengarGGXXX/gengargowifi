...
def hashcat_hiper_multinivel(hash_file, longitud, salida, found_flag, stop_flag, ssid=None, nombre_comercio=None):
    ...
    for nivel_nombre, limite in niveles:
        if found_flag.is_set() or stop_flag.is_set():
            break
        gui_print_rapido(salida, f"🔄 Nivel {nivel_nombre}: generando {limite:,} candidatos híper...\n")
...
def proceso_hiper_multinivel(ssid, bssid, cap_file, salida, longitud_chars, modo_ataque, stop_flag, nombre_comercio=None):
    ...
    if not hashcat_disponible:
        ...
        return
    gui_print_rapido(salida, f"🎯 Target: {ssid} - Contraseñas híper complejas {longitud_chars} chars\n")
    ...
    gui_print_rapido(salida, f"📊 Multinivel: 7 fases de ataque progresivo\n\n")
    if nombre_comercio:
    ...
    try:
        while not found_flag.is_set() and not stop_flag.is_set():
            if modo_ataque == "hiper_multinivel":
                gui_print_rapido(salida, f"🚀 Iniciando ciclo híper multinivel...\n")
...
def crear_gui():
    ventana = tk.Tk()
    ventana.title("🚀 HASHCAT HÍPER MULTINIVEL OPTIMIZADO - IA + Comercio + Ruido")
    ventana.configure(bg=ESTILO["bg"])
    ventana.geometry("1250x850")
    
    header = tk.Label(ventana, text="🚀 HASHCAT HÍPER MULTINIVEL OPTIMIZADO - IA + COMERCIO + RUIDO", 
    ...
    config_frame = tk.LabelFrame(ventana, text="⚙️ Configuración HÍPER MULTINIVEL + COMERCIO", bg=ESTILO["bg"], fg=ESTILO["fg"])
    ...
    tk.Label(config_frame, text="Modo híper:", **ESTILO).grid(row=0, column=2, padx=10, pady=5)
    ...
    def detener_ataque():
        stop_flag.set()
        gui_print_rapido(salida, "\n⏹️ Deteniendo ataque híper...\n")
    
    botones_frame = tk.Frame(ventana, bg=ESTILO["bg"])
    botones_frame.pack(pady=15)
    
    hiper_btn = tk.Button(botones_frame, text="🚀 HASHCAT HÍPER MULTINIVEL + COMERCIO", 
    ...
    sistema_info = tk.Label(ventana, text=f"🎮 {gpu_texto} | {hashcat_info} | Híper + Comercio + Ruido",
                           **ESTILO)
    sistema_info.pack(pady=5)
    
    gui_print_rapido(salida, "🚀 HASHCAT HÍPER MULTINIVEL v3.0 + COMERCIO - IA AVANZADA MÁXIMA\n")
    gui_print_rapido(salida, "🎮 GPU + IA + Análisis contextual + Frecuencia geográfica + Comercio\n")
    gui_print_rapido(salida, "🔬 Niveles: Comercio → Geo → Empresarial → Markov → ML → Evolutivo → Contextual → Híbrido\n")
    gui_print_rapido(salida, "🏪 NUEVO: Patrones comerciales con ruido optimizado\n")
    gui_print_rapido(salida, "🎲 Formato: ruido + empresa + ruido / empresa + ruido / ruido + empresa\n")
    gui_print_rapido(salida, "🧠 Análisis contextual SSID + Frecuencia geográfica\n")
    gui_print_rapido(salida, "⚗️ Patrones empresariales + Markov avanzado (orden 2)\n")
    gui_print_rapido(salida, "🤖 ML predictivo avanzado + N-gramas dinámicos\n")
    gui_print_rapido(salida, "🧬 Algoritmo evolutivo adaptativo + Selección torneo\n")
    gui_print_rapido(salida, "🚫 Validación híper estricta + Anti-patrones masivos\n")
    gui_print_rapido(salida, "📏 Longitud optimizada: 6-16 caracteres\n")
    gui_print_rapido(salida, "🎯 Reduce espacio búsqueda 97-99.5%\n")
    gui_print_rapido(salida, "⚡ Para 12 chars: De milenios a 15-45min\n")
    gui_print_rapido(salida, "🏃‍♂️ Modos: Híper multinivel, Adaptativo completo\n")
    ...
    if verificar_hashcat():
        gui_print_rapido(salida, "⚡ Hashcat disponible - Modo GPU HÍPER activado\n\n")
    else:
